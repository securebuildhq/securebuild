...
if (isEnabled) {
  const serviceName = process.env.DD_SERVICE || 'securebuild-www';
...
const getRoutePattern = (path: string): string => {
  ...
  const routePatterns = [
    { pattern: /^\/api\/custom-build\/[^/]+\/logs$/, replacement: '/api/custom-build/[id]/logs' },
    { pattern: /^\/api\/custom-build\/[^/]+\/status$/, replacement: '/api/custom-build/[id]/status' },
    { pattern: /^\/api\/custom-build\/[^/]+$/, replacement: '/api/custom-build/[id]' },
    { pattern: /^\/api\/v1\/admin\/teams\/[^/]+$/, replacement: '/api/v1/admin/teams/[teamId]' },
    { pattern: /^\/api\/v1\/vendor\/status\/[^/]+$/, replacement: '/api/v1/vendor/status/[buildId]' },
  ];

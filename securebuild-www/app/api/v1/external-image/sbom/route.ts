import { getExternalImageDigestForTag, getExternalImageSBOM, getBatchExternalSboms, getBatchDigestsForTags, type ImageRefTag, BatchSbomResult } from "@/lib/externalimage/externalimage"
...
const batchListSboms = traceFunction('api.external_image.sbom.batchListSboms', async (teamId: string, inputDigests: string[], inputImages: string[], arch: string = 'x86_64'): Promise<ResultEntry[]> => {
  ...
  const sbomResults = allDigests.size > 0
    ? await getBatchExternalSboms([...allDigests], arch)

import { getBatchDigestsForTags, getBatchExternalImageScans, type ImageRefTag, getExternalImageDigestForTag, getExternalImageScan, BatchScanResult } from "@/lib/externalimage/externalimage"
...
const batchListScans = traceFunction('api.external_image.scan.batchListScans', async (teamId: string, inputDigests: string[], inputImages: string[], format: 'raw' | 'parsed', arch: string = 'x86_64'): Promise<ResultEntry[]> => {
  ...
  // Step 4: Get sbom results for all digests in a single query
  const scanResults = allDigests.size > 0
    ? await getBatchExternalImageScans([...allDigests], arch, format)

  ...
  const handleVerifyCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success && result.jwt) {
        ...
        document.cookie = `securebuild_session=${result.jwt}; expires=${expires.toUTCString()}; path=/; SameSite=Lax`
        ...
        <Card className="w-full max-w-md">
            ...
            {!showCodeInput ? (
              <>
                ...
                >
                  ...
                  <span>{isLoading ? "Redirecting..." : "Sign up with Google"}</span>
                </Button>
          ...
          <CardFooter className="flex flex-col space-y-4">
            <div className="text-sm text-center text-muted-foreground">
              ...
              <Link href={searchParams.get('next') ? `/login?next=${encodeURIComponent(searchParams.get('next')!)}` : "/login"} className="text-teal-600 hover:text-teal-700 font-medium">
                Log in
              </Link>
            </div>
          </CardFooter>

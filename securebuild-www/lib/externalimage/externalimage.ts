 ...
 * For each digest, returns scan results and metadata if the team has access.
 * NOTE: We do not require teamId as we do not validate team access.
 *
...
export const getBatchExternalImageScans = traceFunction('lib.externalimage.getBatchExternalImageScans', async (
  ...
  try {
    ...
    // Phase 1: Get scan results (with SBOM metadata and SBOM status)
    // Phase 2: Get SBOM status for digests that don't have scan entries yet

    // Query 1: Get scan results with SBOM data and SBOM status
    const scanQuery = `
      SELECT
        escan.digest,
    ...
    const scanResult = await db.query(scanQuery, [arch, digests])
    ...
    // Query 2: Get SBOM status for digests that don't have scan entries yet (pending/generating SBOM)
    const digestsWithoutScans = digests.filter(d => !resultMap.has(d))
    if (digestsWithoutScans.length > 0) {
      ...
      `
      const sbomStatusResult = await db.query(sbomStatusQuery, [digestsWithoutScans])
  ...
  {
    getTags: (digests: string[], arch: string, format: 'raw' | 'parsed') => ({
 ...
 * For each digest, returns SBOM data and metadata if available.
 * NOTE: We do not require teamId as we do not validate team access.
 *
 * @param digests - Array of image digests to query
 * @param arch - Target architecture (x86_64 or aarch64)
...
export const getBatchExternalSboms = traceFunction('lib.externalimage.getBatchExternalSboms', async (
  ...
  try {
    ...
    // 3. Returns SBOM data and metadata for all requested digests in one query
    ...
    const query = `
      ...
      WHERE
        esbom.arch = $1
        AND esbom.digest = ANY($2)
    `

    const result = await db.query(query, [arch, digests])
  ...
  {
    getTags: (digests: string[], arch: string) => ({
      'args.digests.length': digests.length,
      'args.arch': arch,
    })
  })
